...
def alpha_eff(Isp, n):
    ...
    return mpf(pi*alpha*lambda_eff)
...
def sv_21(s, li, lf, CJ, Ctau, XX):
    ...
    pref = li*( (lf*XX)**5 ) * (2*s + 1)/( gx**2 )
    num = ( 2**12 ) * pi * alpha * ( XX**2 ) * exp(-4*XX*li*arcctg(XX*lf/2))
...
def sv_2p5_2_bsf(XX):
    ...
    num = 41472*alpha*( 128*( (3*CJ + 2*Ctau)**2 ) + ( 24*CJ + Ctau*(4 - 27*XX*XX) )**2 )*(XX**6)
    den = 25*( (4 + 9*XX*XX)**5 ) 
...
np.savetxt('Num_lists/BSF_1s1_TA.txt', ListTA)
...
np.savetxt('Num_lists/BSF_1s3_TA.txt', ListTA)
...
np.savetxt('Num_lists/BSF_1s5_TA.txt', ListTA)
...
np.savetxt('Num_lists/BSF_2s1_TA.txt', ListTA)
...
np.savetxt('Num_lists/BSF_2s3_TA.txt', ListTA)
...
np.savetxt('Num_lists/BSF_2s5_TA.txt', ListTA)
...
np.savetxt('Num_lists/BSF_2p1_TA.txt', ListTA)
...
np.savetxt('Num_lists/BSF_2p3_TA.txt', ListTA)
...
np.savetxt('Num_lists/BSF_2p5_TA.txt', ListTA)
